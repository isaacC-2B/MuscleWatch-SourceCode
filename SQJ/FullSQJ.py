...
try:
    ...
    while True:
        try:
            ...
            if len(bf_data) >= 40 and len(st_data) >= 40:
                combined_data = bf_data[:40] + st_data[:40]
                rpe = categorize_sample(combined_data, max_value)
                print(f"Combined data length: {len(combined_data)}")
                
                model_input = np.array(combined_data).reshape(1, 80)
                ...
                if rpe == 0:
                    ...
                    rep_count += 1
                    lcd.color = [0, 100, 0]
                    lcd.message = f"Proper SQJ\nRCT: {rep_count} RPE: {rpe}"
                    print("Proper SQJ")
                    print(f"Rep Count: {rep_count}")
                else:
                    lcd.clear()
                    lcd.color = [100, 0, 0]
                    lcd.message = f"Improper SQJ\nRCT: {rep_count} RPE: {rpe}"
                    print("Improper SQJ")
                    print(f"Rep Count: {rep_count}")
