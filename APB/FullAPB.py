...
try:
    ...
    while True:
        try:
            ...
            if len(bf_data) >= 80 and len(st_data) >= 80:
                combined_data = bf_data[:80] + st_data[:80]
                rpe = categorize_sample(combined_data, max_value)
                print(f"Combined data length: {len(combined_data)}")
                
                model_input = np.array(combined_data).reshape(1, 160)
                ...
                if rpe == 0:
                    ...
                    rep_count += 1
                    lcd.color = [0, 100, 0]
                    lcd.message = f"Proper APB\nRCT: {rep_count} RPE: {rpe}"
                    print("Proper APB")
                    print(f"Rep Count: {rep_count}")
                else:
                    lcd.clear()
                    lcd.color = [100, 0, 0]
                    lcd.message = f"Improper APB\nRCT: {rep_count} RPE: {rpe}"
                    print("Improper APB")
                    print(f"Rep Count: {rep_count}")
